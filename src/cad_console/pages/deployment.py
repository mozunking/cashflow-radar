        ...
        with st.container():
            ...
            with col_b:
                status_color = (
                    ...
                    else "orange" if model["status"] == "Staging" else "gray"
                )
    ...
    with st.form("upload_model"):
        ...
        with col2:
            model_type = st.selectbox(
                "Model Type", ["iForest", "LOF", "Graph", "AutoEncoder", "Ensemble"]
            )
        ...
        if submitted:
            if model_name and model_version and upload_file:
                st.success(
                    f"Model {model_name} v{model_version} uploaded successfully!"
                )
            else:
...
def _render_configuration():
    ...
    with st.expander("Model Configuration"):
        ...
        with col1:
            st.number_input(
                "iForest n_estimators", value=100, min_value=10, max_value=500
            )
